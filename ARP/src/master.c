...
int spawn(const char * program, char * arg_list[], int pipes[][2], int index) {
  ...
  if(child_pid < 0) {
    ...
    // Duplicate stdin/stdout for communication
...
int main() {
  ...
  pid_t pid_s = spawn("./bin/server", NULL, pipes, 0);
  pid_t pid_d = spawn("./bin/drone", NULL, pipes, 1);
  pid_t pid_ts = spawn("./bin/targets", NULL, pipes, 2);
  pid_t pid_os = spawn("./bin/obstacles", NULL, pipes, 3);
  pid_t pid_wd = spawn("./bin/watch_dog", NULL, pipes, 4);
  pid_t pid_w = spawn("/usr/bin/konsole", arg_list_window, pipes, 5);
